...
void LeafData::addSecondBarcode(string barcode){
    bool barcodeFound=false;
    for (int i=0; i<mSecondBarcodes.size(); ++i){
	if (mSecondBarcodes[i]==barcode){
            mSecondBarcodeCounts[i]++;
	    barcodeFound=true;
	    break;
	}
    }
    if (barcodeFound==false){
    	mSecondBarcodes.push_back(barcode);
	mSecondBarcodeCounts.push_back(1);
    }
}
...
vector <string> LeafData::secondBarcodes(){
    return mSecondBarcodes;
}
vector <int> LeafData::secondBarcodeCounts(){
    return mSecondBarcodeCounts;
}
